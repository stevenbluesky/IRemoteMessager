<#include "../_head0.ftl"/>
    ...
    <div class="col-md-10">
      <div class="row">
          <div class="text-center"><h1>事件组定义</h1></div>
          <hr>
    ...
    <button style="float: right;" type="button" class='btn btn-default' onclick="window.location.href='../eventgroup/addeventgrouppage'">增加</button>

    <button style="float: right;" type="button" class="btn btn-default" onclick='eventGroupEvent()'>事件组事件</button>
	...
	<table id="table" data-toggle="table" style="table-layout: fixed;">
        ...
        <tr>
            ...
            <th data-field="description"  class="text-center col-md-4" >说明</th>
        </tr>
        </thead>
    </table>
      </div>
    ...
    <script type="text/javascript">
        ...
        $('#table').bootstrapTable({
            url: '../eventgroup/showeventgrouplist',
            method: 'GET',                      //请求方式（*）
        ...
        function getCheckedId() {
            ...
            for (var index in pid) {
                ids.push(pid[index].msgeventgroupid);
            }
            if (ids.length == 0) {
                return new Array("-1");
            }
        ...
        function modifyEventGroup(){
            var ids = getCheckedId();
            if (ids[0] == null || ids[0] == ""|| ids[0] == "-1") {
                spop({template: '请先选择一个事件组！', position: 'top-center', style: 'warning', autoclose: 2000});
                return;
            }
            if(ids.length>1) {
        ...
        function deleteEventGroup(){
            var ids = getCheckedId();
            if (ids[0] == null || ids[0] == ""|| ids[0] == "-1") {
                spop({template: '请至少选择一个事件组！', position: 'top-center', style: 'warning', autoclose: 2000});
                return;
            }
            $.confirm({
                title: '确认',
                ...
                buttons: {
                    ok: {
                        ...
                        action: function() {
                            $.ajax({
                                type: 'post',
                                url: '../eventgroup/deleteeventgroups',
        ...
        function eventGroupEvent(){
            ...
            if(ids.length>1) {
                ...
                window.location.href='../eventgroup/eventgroupevent?msgeventgroupid='+ids[0];
            }
        }
